...
def valid_reality_public_key(value: str) -> bool:
    if not value or not re.fullmatch(r"[A-Za-z0-9_-]{43,44}", value.strip()):
        return False
    try:
        raw = _decode_base64url_nopad(value.strip())
    except Exception:
        return False
    return len(raw) == 32
...
def normalize_short_id(value: str) -> str:
    ...
    if not s:
        return ""
    if re.fullmatch(r"[0-9a-f]{1,16}", s):
        return s
    return ""
...
def build_vless_outbound(link: str) -> Dict:
    ...
    if security in {"tls", "reality"}:
        ...
        if str(params.get("allowInsecure", "false")).lower() in {"1", "true", "yes", "on"}:
            tls["insecure"] = True
        if security == "reality":
            # sing-box requires uTLS for REALITY client. If the source link
            # doesn't specify a fingerprint, use Chrome explicitly.
            tls.setdefault("utls", {"enabled": True, "fingerprint": "chrome"})
            public_key = (params.get("pbk") or params.get("publicKey") or "").strip()
            if not valid_reality_public_key(public_key):
                raise ValueError("invalid reality public_key")
            tls["reality"] = {"enabled": True, "public_key": public_key}
            short_id = normalize_short_id(params.get("sid") or params.get("shortId") or "")
            if short_id:
...
def main() -> None:
    ...
    meta = {
        ...
        "schema_note": "Updated for sing-box 1.13+ route actions and default_domain_resolver",
    }
